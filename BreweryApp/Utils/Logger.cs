...
namespace ResotelApp.Utils
{
    ...
    static class Logger
    {
        ...
        private static void _createsLogDirIfNeeded()
        {
            ...
            _logDir = _logDir.Replace("#MyDocuments#", myDocumentsDir);
        ...
        static Logger()
        {
            ...
            _fileName = $"{Path.Combine(_logDir, $"breweryApp_{DateTime.Now:ddMMyyyy}.log")}";
            _errorFileName = $"{Path.Combine(_logDir, $"breweryApp{DateTime.Now:ddMMyyyy}.err.log")}";
            _createsLogDirIfNeeded();
        }
